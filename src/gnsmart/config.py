        ...
        with open(ttl) as f:
            ...
            for content in contents:
                ...
                if content.startswith("@"):
                    ...
                    prefix = [new_content for new_content in content_list if ':' in new_content and not ('/' in new_content or '\\' in new_content)]
                    prefix = prefix[0] if len(prefix) >= 1 else prefix
                    if prefix not in prefixes:
                        prefixes.append(prefix)
                elif len(content) != 0:
                    content_list = content.split()
                    predicate = [new_content for new_content in content_list if ':' in new_content and not ('/' in new_content or '\\' in new_content)]
                    predicate = predicate[0] if len(predicate) >= 1 else predicate

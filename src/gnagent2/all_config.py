...
def match_checker(
    ...
    relevance = f"{context}\nHow confident are you that the answer of the AI system addressed the query?"
    relevance_score = judge(question=relevance).get("answer")

    formulation = f"{context}\nHow confident are you that the answer of the AI system to the query is well elaborated and detailed?"
    formulation_score = judge(question=formulation).get("answer")

    contradiction = f"{context}\nHow confident are you that the answer generated by the AI system is free from contradiction?"
    contradiction_score = judge(question=contradiction).get("answer")

    irregularity = f"{context}\nHow confident are you that the answer generated by the AI system is free from missing values and irregularities?"
    irregularity_score = judge(question=irregularity).get("answer")

    cloudiness = f"{context}\nHow confident are you that there is no superficial or useless information in the answer generated by the AI system?"
